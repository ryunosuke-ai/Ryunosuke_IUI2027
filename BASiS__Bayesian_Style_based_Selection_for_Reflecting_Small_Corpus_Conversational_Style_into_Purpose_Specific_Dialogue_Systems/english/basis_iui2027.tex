...
One line of work makes desired dialogue behavior explicit and uses it to control response generation. DialGuide represents each guideline as a natural-language condition specifying when it applies and an action specifying what the response should contain \cite{gupta2023dialguide}. It retrieves guidelines relevant to the current dialogue context and uses them to generate or verify a response. GuidedTOD instead represents a sequence of operational guidelines as a Chained Prior, a Markov chain that is updated as the dialogue progresses and used to rerank candidates during inference \cite{wen2025guidedtod}. These studies show that an explicit response policy can guide generation and selection according to the current stage of a dialogue. However, the policy itself must be written by developers and redesigned when the desired dialogue progression changes.

A second line of work derives representations of dialogue progression from corpora. Ganesh et al. predict the teacher's next talk move from the preceding classroom dialogue and talk moves \cite{ganesh2021teacher}. EmoDynamiX represents discourse-level relationships between fine-grained user emotion states and system response strategies as a heterogeneous graph \cite{wan2025emodynamix}. Rudolph et al. estimate dialogue-act transition patterns from counseling conversations and use them to regularize next-dialogue-act prediction toward the observed dialogue flow \cite{rudolph2026transition}. Together, these studies establish that dialogue progression can be represented as corpus-derived states, actions, strategies, and transitions rather than only as manually written rules. Their models, however, are used primarily to predict the next action or strategy; the derived structure is not used to train an LLM to generate natural-language responses, nor is its realization in generated responses directly evaluated.

A third line of work selects training examples from a large candidate pool to adapt LLM behavior efficiently. LIMA emphasizes careful curation for data quality and diversity \cite{zhou2023lima}. AlpaGasus filters instruction data using LLM-based quality scores, whereas DEITA combines instruction complexity, response quality, and diversity-aware selection \cite{chen2023alpagasus,liu2024deita}. LESS estimates the influence of individual examples on downstream performance and selects those expected to be most useful \cite{xia2024less}. These studies show that a criterion representing the desired behavior or capability can guide the construction of effective adaptation data. Their selection criteria, however, do not encode dialogue progression learned from a small reference corpus.

Taken together, prior work provides three elements needed by BASiS: explicit dialogue policies can guide response generation, dialogue progression can be recovered as structure from a corpus, and criterion-based selection can identify useful data for LLM adaptation. These elements have not been connected in a single pipeline. BASiS links them as \emph{corpus-derived dialogue progression $\rightarrow$ training-data selection criterion $\rightarrow$ LLM adaptation}: it induces a probabilistic progression model from a small, high-quality corpus, uses that model to score responses in a large general-purpose corpus, and adapts an LLM with the resulting preference data. This connection enables corpus-derived interactional knowledge to be reflected in generated responses without requiring developers to manually specify a complete dialogue policy.
