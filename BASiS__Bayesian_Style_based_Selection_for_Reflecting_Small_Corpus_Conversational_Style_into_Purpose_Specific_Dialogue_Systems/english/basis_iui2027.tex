...
In the oracle evaluation, an LLM serves as the evaluator and rates how well each model response reproduces the conversational style characteristic of the target corpus. The evaluator is not shown the model names, and the presentation order of the four responses is randomized. We use GPT-5.4 as the oracle LLM with a temperature of 0 to reduce output variability.
...
Assignments will nevertheless ensure that responses from each condition for every prompt are rated by multiple evaluators. The same evaluator will rate the four conditions for a given prompt, preserving a paired comparison across conditions.
\end{comment}
...
\begin{figure}[t]
  ...
  \caption{Human-evaluation results for the three corpus pairs. Results for Base, Target-only, BASiS, and Random on each question will be inserted after the human evaluation is complete.}
...
We evaluated BASiS on three distinct types of purpose-specific dialogue: emotional support, mathematics tutoring, and medical history-taking. BASiS had the highest mean on all 21 oracle-evaluation axes and significantly outperformed Base, Target-only, and Random on 18, 13, and 15 axes, respectively. Target-only did not significantly outperform Base on any axis, so direct training on the small target corpus alone did not yield consistent improvements. The significance pattern varied by domain and axis: improvements were broad in emotional support and mathematics tutoring, whereas response quality and safety showed different patterns in medical history-taking. We discuss the implications of these results below.
