...
\begin{tikzpicture}[font=\sffamily, every node/.append style={outer sep=0pt}]
  ...
  \node[
    ...
    inner ysep=1.5pt,
    font=\sffamily\bfseries\scriptsize
  ] at (0.22,4.35) {A \quad DIALOGUE PROGRESSION MODELING};
  \node[
    anchor=west,
    text=teal,
    fill=slatefill,
    ...
    inner ysep=1.5pt,
    font=\sffamily\bfseries\scriptsize
  ] at (0.22,1.05) {B \quad DATA SELECTION \& ADAPTATION};

  \node[corpus, text width=2.35cm] (small) at (1.48,2.62) {%
    {\color{blue}\large\faDatabase}\\[-1pt]
    {\scriptsize PROGRESSION REFERENCE}\\[-1pt]
  ...
  \node[scorecard, text width=3.45cm] (scoring) at (6.95,-0.62) {%
    {\bfseries\small Dialogue Progression\\Scoring}\\[2pt]
  ...
  \node[
    ...
    \textbf{DPO-trained\\LoRA adapter}
  };

  % Number badges make the four steps easy to map to the method text.
  ...
  \draw[flow, draw=orange]
    (adaptation.east)
    -- node[arrowlabel, midway, yshift=2.5pt, fill=slatefill, inner xsep=0.7pt,
      font=\sffamily\fontsize{6.3}{6.6}\selectfont] {trained adapter}
    (local.west);
\end{tikzpicture}
\end{document}
