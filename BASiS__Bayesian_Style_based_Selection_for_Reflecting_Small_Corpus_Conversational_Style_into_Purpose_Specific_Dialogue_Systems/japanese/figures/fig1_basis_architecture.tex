...
\begin{tikzpicture}[font=\sffamily, every node/.append style={outer sep=0pt}]
  ...
  \node[modelcard, text width=2.82cm] (analysis) at (4.52,2.62) {%
    {\bfseries\small Dialogue Feature\\Analysis}\\[2pt]
    {\scriptsize states \(\bullet\) strategies\\transitions}
  };

  \node[modelcard, text width=3.05cm] (modeling) at (8.78,2.62) {%
  ...
  \node[scorecard, text width=3.45cm] (scoring) at (6.05,-0.62) {%
    {\bfseries\small Style Compatibility\\Scoring}\\[2pt]
    {\scriptsize \(\mathrm{Score}(r_t)\in[0,1]\)}
  };

  \node[adaptcard, text width=4.40cm] (adaptation) at (11.20,-0.62) {%
  ...
  \draw[flow] (analysis.east) -- node[
    arrowlabel,
    font=\sffamily\tiny,
    inner xsep=1.5pt
  ] {dialogue\\features} (modeling.west);
