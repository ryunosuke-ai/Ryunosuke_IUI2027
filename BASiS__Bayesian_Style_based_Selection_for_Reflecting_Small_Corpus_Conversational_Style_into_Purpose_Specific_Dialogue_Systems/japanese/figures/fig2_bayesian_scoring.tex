...
\tikzset{
  ...
  stepbadge/.style={
    ...
    inner sep=0pt,
    text=white,
    font=\sffamily\bfseries\fontsize{6.2}{6.5}\selectfont
  },
  flow/.style={
...
\begin{tikzpicture}[font=\sffamily, every node/.append style={outer sep=0pt}]
  % Five compact panels mirror the internal operations of Step 3.
  ...
  % Panel titles and lettered badges distinguish these operations from Steps 1--4.
  ...
  \node[anchor=north, font=\sffamily\bfseries\scriptsize, text=ink]
    at ($(score.north)+(0,-0.20)$) {PROGRESSION SCORE};

  \node[stepbadge, fill=blue] at ($(prior.north west)+(0.14,-0.15)$) {(a)};
  \node[stepbadge, fill=blue] at ($(predict.north west)+(0.14,-0.15)$) {(b)};
  \node[stepbadge, fill=orange] at ($(observe.north west)+(0.14,-0.15)$) {(c)};
  \node[stepbadge, fill=teal] at ($(update.north west)+(0.14,-0.15)$) {(d)};
  \node[stepbadge, fill=teal] at ($(score.north west)+(0.14,-0.15)$) {(e)};

  % Panel (a): prior state distribution b_{t-1}.
  ...
  % Panel (b): transition-model prediction.
  ...
  % Panel (c): candidate response is mapped to an observation label.
  \node[
    rounded corners=3pt,
    draw=orange,
    fill=white,
  ...
  % Panel (d): the two variables enter separate terms in the Bayesian update.
  ...
  % Panel (e): sum only the target-state posterior mass.
