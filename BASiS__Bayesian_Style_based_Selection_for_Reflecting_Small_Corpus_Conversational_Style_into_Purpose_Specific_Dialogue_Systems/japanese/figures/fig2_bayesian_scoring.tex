...
\begin{tikzpicture}[font=\sffamily, every node/.append style={outer sep=0pt}]
  ...
  \node[panel, minimum width=3.45cm] (predict) at (5.22,2.72) {};
  ...
  \draw[draw=line, line width=0.6pt] (1.72,1.10) -- (1.72,4.16);

  \node[anchor=east, align=right, font=\scriptsize] at (1.57,3.78)
    {Emotion\\disclosure};
  \fill[blue] (1.76,3.66) rectangle ++(0.66,0.24);
  \node[anchor=west, font=\scriptsize] at (2.52,3.78) {0.30};

  \node[anchor=east, align=right, font=\scriptsize] at (1.57,2.82)
    {Emotion\\processing};
  \fill[blue] (1.76,2.70) rectangle ++(0.99,0.24);
  \node[anchor=west, font=\scriptsize] at (2.85,2.82) {0.45};

  \node[anchor=east, align=right, font=\scriptsize] at (1.57,1.86)
    {Considering\\solutions};
  \fill[blue] (1.76,1.74) rectangle ++(0.55,0.24);
  \node[anchor=west, font=\scriptsize] at (2.41,1.86) {0.25};

  % Step 2: transition-model prediction.
  \node[chipblue, align=center] at (5.22,4.42)
  ...
  \node[align=center, font=\scriptsize] at (5.22,2.57) {%
    \(\displaystyle
      \begin{aligned}
        \hat b_t(s_t)
        &=\sum_{s_{t-1}\in S}P(s_t\mid s_{t-1})\\[-1pt]
        &\hspace{8mm}{}\cdot b_{t-1}(s_{t-1})
      \end{aligned}
    \)
  };
  ...
  \node[
    ...
    font=\scriptsize,
    text=muted
  ] at (8.97,3.35) {candidate\\response \(r_t\)};
  \node[
    circle,
    draw=orange,
    fill=orangefill,
  ...
  \node[chiporange] (obsport) at (11.55,4.55)
    {observation \(o_t\)};
  \node[chipblue] (predport) at (14.35,4.55)
    {prediction \(\hat b_t\)};
