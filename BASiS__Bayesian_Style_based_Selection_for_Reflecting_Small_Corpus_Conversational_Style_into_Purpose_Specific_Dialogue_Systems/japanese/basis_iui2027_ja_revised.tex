...
適合度スコアは0から1までの値を取り，値が大きいほど，候補応答によって目的コーパスに特徴的な状態へ移行する可能性が高いことを表す．例えば，感情支援では感情の整理や適切な解決策の検討，数学チュータリングでは学習者自身による推論の進展，医療問診では必要な症状情報の段階的な整理などを，対象領域に応じて$S_{\mathrm{target}}$として設定できる．
...
ESConvとDailyDialogを用いた実験では，感情支援対話のスタイルと会話進行を捉えるため，表\ref{tab:oracle_axes}に示す七つの評価軸を用いた．これらの評価軸は，ESConvに特徴的な共感的支援スタイルの強さと語調に加え，支援者としての役割，非指示的な応答方針，応答戦略と会話段階の適合，助言を行うタイミング，および日本語対話としての自然さを評価する\cite{mir2019styletransfer,liu2021esconv,zhang2018persona,deng2023mixedinitiative}．
...
MediTODとWildChat-1Mを用いた実験では，病歴聴取の適切さ，応答品質，および医学的安全性を捉えるため，表\ref{tab:oracle_axes}に示す七つの評価軸を用いた\cite{saley2024meditod}．これらの評価軸では，直前までの症状や背景に即した応答，医療対話としての総合的な品質，情報不足時に診断や治療方針を急がないこと，不確実性の扱い，患者にとっての理解しやすさ，および根拠の乏しい診断や危険な医学的助言の回避を評価する．
