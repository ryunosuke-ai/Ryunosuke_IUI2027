...
\begin{abstract}
本論文では，小規模で高品質なコーパスに基づくベイズ的データ選別を通じて，目的特化対話に求められる会話スタイルへローカル大規模言語モデル（LLM）を適応させる手法BASiS（Bayesian Style-based Selection）を提案する．近年のLLMは，流暢かつ正確な応答を生成できるようになった．一方，感情支援，数学チュータリング，医療問診などの場面では，正しい内容を提示するだけでなく，適切な方法で応答することが求められる．このような会話スタイルは，小規模な対話コーパスに表れていることが多いが，それらをそのままLLMの学習に用いるにはデータ量が限られている．大規模な汎用対話コーパスは十分なデータ量を持つ一方で，目的の会話スタイルに適合する事例を十分に含んでいるとは限らない．
BASiSは，小規模コーパスから会話状態，応答戦略，状態遷移を抽出し，それらをベイズ的対話モデルとして表現する．次に，大規模コーパスに含まれる対話を，目的の会話スタイルへの適合度に基づいてスコアリングし，学習データを選別する．さらに，選別したデータから選好ペアを構築し，Direct Preference Optimization（DPO）を用いてLLMを学習する．BASiSの主な利点は，小規模コーパスに暗黙的に含まれる対話の進め方を，より大規模なコーパスから適切なデータを特定するための選別基準へ変換する点にある．これにより，詳細な対話ガイドラインを人手で設計する負担を軽減しながら，一般的な品質や話題の類似性だけでは捉えにくい応答のタイミングや複数ターンにわたる対話の進行を，データ選別に反映できる．また，単一の応答戦略に固定するのではなく，会話状態を確率分布として保持することで，同一の文脈に対する複数の妥当な応答戦略を維持できる．
感情支援，数学チュータリング，医療問診を対象として，学習前のベースモデル，BASiSによる選別データで学習したモデル，および目的コーパスのカテゴリ内から無作為に選別したデータで学習したモデルを比較することで，BASiSを評価した．その結果，三つの実験設定すべてにおいて，BASiSによる選別データで学習したモデルが，目的コーパスに特徴的な応答方針と対話の進行を再現する評価で最も高いスコアを示した．
\end{abstract}
