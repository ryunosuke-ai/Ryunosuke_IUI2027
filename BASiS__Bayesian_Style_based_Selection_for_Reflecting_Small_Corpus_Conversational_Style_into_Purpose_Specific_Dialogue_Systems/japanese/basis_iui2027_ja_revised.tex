...
本論文の構成は以下の通りである．第\ref{sec:relwork}章では，ガイドラインに基づく応答制御，対話履歴に基づく対話行動・応答戦略予測，およびLLMの学習データ選別に関する関連研究を紹介する．第\ref{sec:method}章では，提案手法BASiSの全体構成と各モジュールの機能を述べる．第\ref{sec:evaluation}章では，BASiSの有効性を検証する実験，実験結果，および考察について述べる．最後に，第\ref{sec:conclusion}章で本研究をまとめる．
...
一方で，各プロンプトに対する各条件の応答が複数名の評価者によって評価されるように割り当てを行う．同一の評価者が，同一のプロンプトに対する四条件の応答を評価することで，条件間の対応を保った評価を行う．
\end{comment}
...
すべての質問項目について，1を「まったくそう思わない」，7を「非常にそう思う」とする．評価者には，四条件のモデル名や学習方法を知らせず，応答内容のみに基づいて評価を求める．これにより，モデルに対する事前の印象や，提案手法であることを知ることによる評価への影響を抑える．
...
人手評価の実施時期は未定である．将来実施する場合は，表\ref{tab:human_items}に示した質問項目について，Base，Target-only，BASiS，Randomの四条件を比較し，Oracle評価と同様にコーパスペアごとの結果を報告する．また，Oracle評価と人手評価の傾向が一致するかを確認するとともに，複数の人間評価者間の一致度についても分析する．
...
\begin{figure}[t]
  ...
  \caption{三種類のコーパスペアにおける人手評価結果．人手評価の完了後，各質問項目に対するBase，Target-only，BASiS，Randomの評価結果を挿入する予定である．}
...
本実験では，感情支援，数学チュータリング，医療問診という性質の異なる三種類の目的特化対話を対象としてBASiSを評価した．Oracle評価では，BASiSが合計21の評価軸すべてで最高の平均スコアを示し，Base，Target-only，Randomに対して，それぞれ18軸，13軸，15軸で有意に高かった．一方，Target-onlyはBaseを有意に上回る軸がなく，小規模目的コーパスを直接学習するだけでは一貫した改善が得られなかった．有意差の現れ方は領域と評価軸によって異なり，感情支援と数学チュータリングでは広範な改善が確認されたのに対し，医療問診では応答品質と安全性の各側面で異なる傾向が見られた．以下では，それぞれの結果が示す意味について考察する．
...
異なる性質を持つ三種類の目的特化対話を対象とした比較実験では，BASiSが全21評価軸で最も高い平均評価を示した．実験結果から，BASiSは，共感的な応答と助言のタイミング，学習者の理解状態に応じた教育的支援，および不確実な状況で断定を避ける応答方針を，それぞれの目的に応じてモデルへ反映できる可能性が示された．目的コーパスのみで学習したTarget-onlyがBaseを有意に上回る軸はなく，本実験条件では小規模コーパスを直接学習するだけでは一貫した改善に不十分であった．また，粗いキーワードカテゴリ内から無作為に選別したRandomとの比較から，話題の一致だけでなく，目的の会話スタイルに基づいて学習データを選別することの重要性が示唆された．
